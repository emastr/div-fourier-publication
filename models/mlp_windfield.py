...
class MLPWindfield(wf.Windfield):
    """ Create a neural network Windfield model (MLP stands for Multi-Layer Perceptron).

    Example usage and public attributes:

    Create a new MLPWindfield model:

    mlp_wf = MLPWindfield(epochs = 600,
                        learning_rate = 0.001,
                        layers = [20,20,20,2],
                        activation = 'relu',
                        l2 = 0.01,
    ...
    There are a lot of keywords to unpack in the above example:
    ------------------------------------------------------------------------------------------------------------------

    'learning_rate' - float (default 0.001) which determines how fast the network trains.

    'layers' - an array (default( [20,20,20,2]) holding the sizes of the hidden layers as well as the output
               layer of the network.

    'activation' - string, i.e 'relu', or 'sigmoid', indicating which activation function to use.

    'l2' - float (default 0.01) parameter used in L^2-norm regularization of the hidden layers of the network.

    'batch_size' - int (default 32) determining the size of each mini batch in the adam optimization algorithm.

    'elevation' - boolean (default True)

    'gamma' - float (default 0.01) used in divergence-constraint regularization (determines the relative importance
              of keeping the divergence of the windfield small).

    'N' - int (default 10) determining how many times throughout training to calculate test loss (i.e the generaliation
          error).

    ------------------------------------------------------------------------------------------------------------------
    Many of these keyword parameters can be tweaked to optimize the performance of the neural network, but all of
    them can be left to their default settings, which should give reasonable performance. Hence, it is possible to
    simply make a model using only:

    mlp_wf = MLPWindfield().

    To use elevation data, a lookup needs to be added:

    mlp_wf.altitude_lookup = MLPWindfield.make_altitude_lookup(wind_data).

    To fit the model to data, simply use the .fit()-method:

    mlp_wf.fit(wind_data).

    To make wind prediction in coordinates x and y, use:
    ...
    def __init__(self, epochs = 600, learning_rate = 0.001, layers = [20,20,20,2], activation = 'relu',
                 l2 = 0.01, decay = 0.0, dropout_rate = 0.0, batch_size = 32, elevation = True, gamma = 0.01, N = 10):
